...
public class TestArrayList {
	...
	public void testListIterator() {
		...
		ListIterator<Character> itr = l1.listIterator();
		...
		assertEquals(Character.valueOf('a'), itr.previous());
		assertFalse(itr.hasPrevious());
		
		itr.add('z');
		assertEquals(4, l1.size());
		assertEquals(Character.valueOf('z'), l1.get(0));
		
		assertTrue(itr.hasNext());
		assertEquals(Character.valueOf('a'),itr.next());
		itr.set('y');
		assertEquals(Character.valueOf('y'), l1.get(1));
	}
